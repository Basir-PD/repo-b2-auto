      ...
      <div className={isScrolled ? 'w-full' : 'container mx-auto max-w-5xl'}>
        <div className={`px-6 sm:px-8 lg:px-12 flex items-center justify-between transition-all duration-300 relative z-50 ${
          isScrolled 
            ...
            : 'h-20 glass-dark rounded-2xl hover:scale-[1.01] hover:shadow-2xl hover:shadow-green-900/20'
        }`}>
          {/* Logo */}
          <Link href="/" className="flex items-center gap-3 group">
            <div className={`rounded-lg -skew-x-6 flex items-center justify-center shadow-lg shadow-green-500/20 group-hover:rotate-3 transition-all duration-300 ${
              isScrolled ? 'w-8 h-8 bg-green-600' : 'w-10 h-10 bg-green-600'
            }`}>
          ...
          <nav className="hidden md:flex items-center gap-8">
            <Link href="#services" className={`text-sm font-bold uppercase tracking-wide transition-colors ${
              isScrolled ? 'text-slate-700 hover:text-green-600' : 'text-slate-300 hover:text-green-400'
            }`}>
              {t.header.services}
            </Link>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative hidden md:block">
              ...
              >
                <span className={language === 'en' ? "text-green-600" : isScrolled ? "text-slate-400" : "text-slate-400"}>EN</span>
                <span className={isScrolled ? "text-slate-400" : "text-slate-600"}>/</span>
                <span className={language === 'fr' ? "text-green-600" : isScrolled ? "text-slate-400" : "text-slate-400"}>FR</span>
            ...
            >
              <Phone className="w-4 h-4 text-green-600 group-hover:animate-bounce" />
            ...
            <button 
              ...
              className={`md:hidden p-2 transition-colors z-50 relative ${
                isScrolled ? 'text-slate-900 hover:text-green-600' : 'text-white hover:text-green-400'
              }`}
            >
