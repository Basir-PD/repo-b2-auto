 ...
 *
 * The vehicle figure has moved twice: 2 000 until 2026-09-09, then 10 000,
 * then 7 500 on 2026-09-16 when the owner corrected it again. It lives in
 * `siteConfig.facts` and is read from there by the homepage stat row, the
 * about page, a photo caption and this band, so the four cannot drift
 * apart — which they had, silently, before this.
 *
...
export function businessStats(lang: Lang) {
  ...
  /*
    ...
    into view, and a counter needs a number, not "11 ans". The unit is printed
    straight after the formatted figure — which is also how the vehicle count
    gets its "+" without the counter trying to animate toward a string.
